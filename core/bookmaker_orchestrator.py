...
class BookmakerOrchestrator:
    ...
    def start(self) -> None:
        ...
        self.logger.info(
            f"Expected throughput: ~{len(self.bookmaker_processes) * 5} items/sec"
        )
        self.logger.info("=" * 60)
    ...
    def stop(self) -> None:
        ...
        for process in self.bookmaker_processes:
            if process.is_alive():
                ...
                if process.is_alive():
                    self.logger.warning(
                        f"⚠️  {process.name} didn't stop, terminating..."
                    )
                    process.terminate()
    ...
    def _log_final_stats(self) -> None:
        ...
        if self.db_worker:
            ...
            self.logger.info(
                f"Avg throughput:    {db_stats['items_per_second']:.1f} items/sec"
            )
