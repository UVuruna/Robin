...
# 
# Koristi trenirani model iz models/digit_cnn.pth
...
class FastCNNOCR:
    ...
    def __init__(self, model_path: str = "data/models/digit_cnn.pth"):
        ...
        # Config - FIXED za Aviator font
        self.window_size = (28, 28)
        self.stride = 16  # POVEĆANO: bio 6, sada 16 (3x manje detections)
        self.min_confidence = 0.92  # POVEĆANO: bio 0.80, sada 0.92 (strože)
    ...
    def _nms(self, detections: List[Detection]) -> List[Detection]:
        """
        Non-maximum suppression sa BOLJOM logikom.
        
        Proverava:
        1. Horizontalno rastojanje (mora biti >15px za različite cifre)
        2. Vertikalno rastojanje (mora biti <10px za istu liniju)
        """
        if len(detections) <= 1:
            return detections
        ...
        while detections:
            ...
            # Remove overlapping ili loše pozicionirane
            filtered = []
            for d in detections:
                x_dist = abs(d.position[0] - best.position[0])
                y_dist = abs(d.position[1] - best.position[1])
                
                # Zadrži samo ako je:
                # - Dovoljno daleko horizontalno (>15px) ILI
                # - Previše daleko vertikalno (>10px, znači druga linija)
                if x_dist > 15 or y_dist > 10:
                    filtered.append(d)
            
            detections = filtered
        
        # Filter dodatno: drži samo one na ISTOJ liniji (y pozicija blizu)
        if keep:
            # Median y pozicija
            median_y = sorted([d.position[1] for d in keep])[len(keep) // 2]
            
            # Drži samo one u rasponu ±10px od median_y
            keep = [d for d in keep if abs(d.position[1] - median_y) < 12]
    ...
    def recognize(self, img: np.ndarray, debug: bool = False) -> str:
        ...
        # DODATNA VALIDACIJA: Aviator score ima 3-5 cifara
        # Ako ima više od 6 detections, nešto ne valja
        if len(detections) > 6:
            if debug:
                print(f"⚠️  Previše detections ({len(detections)}), uzimam top 5")
            # Uzmi top 5 po confidence
            detections = sorted(detections, key=lambda d: d.confidence, reverse=True)[:5]
            # Sortiraj nazad po x
