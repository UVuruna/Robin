...
class GUIController:
    ...
    PYAUTOGUI_SETTINGS = {"PAUSE": 0.1, "FAILSAFE": True, "MINIMUM_DURATION": 0.1}

    # CRITICAL: Pause after bet execution to release mouse control
    BET_COMPLETION_PAUSE = 2.0  # seconds

    def __init__(self, betting_queue):
    ...
    def start(self) -> None:
        ...
        self.controller_thread = threading.Thread(
            target=self._controller_loop, name="GUIController", daemon=False
        )
    ...
    def stop(self) -> None:
        ...
        self.logger.info(
            f"GUI Controller stopped. Bets placed: {self.total_bets_placed}, Errors: {self.total_errors}"
        )
    ...
    def _process_betting_request(self, request: BettingRequest) -> None:
        self.logger.info(
            f"Processing bet for {request.bookmaker_name}: {request.bet_amount}"
        )
        ...
        try:
            ...
            self.total_errors += 1
            self.logger.error(
                f"Error placing bet for {request.bookmaker_name}: {e}", exc_info=True
            )
    ...
    def _execute_bet(
        ...
        try:
            ...
            self.logger.debug(
                f"Executed bet: amount={amount}, coords={play_amount_coords}, button={play_button_coords}"
            )
            return True
