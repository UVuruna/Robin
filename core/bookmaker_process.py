...
class BookmakerProcess(Process):
        ...
        try:
            ...
            while not self.shutdown_event.is_set():
                if self.current_money >= self.target_money:
                    self.logger.info(
                        f"Target reached! Current: {self.current_money}, Target: {self.target_money}"
                    )
                    break
    ...
    def _state_machine_loop(self) -> None:
        ...
        if (
            ...
            and self.previous_phase != GamePhase.ENDED
        ):
        ...
        # STATE 2: LOADING - Between rounds
            ...
            if (
                ...
                and not self.bet_placed_for_current_round
            ):
    ...
    def _handle_round_end(self) -> None:
        ...
        try:
            ...
            if result:
                ...
                self.current_bet_index = (self.current_bet_index + 1) % len(
                    self.bet_sequence
                )
    ...
    def _place_bet_once(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Bet placed: {bet_amount} RSD (index: {self.current_bet_index})"
            )
        except queue.Full:
    ...
    def _collect_snapshot(self) -> None:
        ...
        try:
            ...
            if (
                not self.round_snapshots
                or self.round_snapshots[-1]["current_score"]
        ...
        try:
            ...
            for attempt in range(3):
                try:
                    ...
                    self.logger.warning(
                        f"Attempt {attempt + 1} to read balance failed: {e}"
                    )
