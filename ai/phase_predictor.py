    ...
    def start_prediction(self, bookmakers: list, delay: float = 0.5):
        ...
        try:
            while True:
                ...
                self.logger.info(f"{bookmaker:15s} -> RGB{rgb} -> Cluster {cluster}")
...
def main():
    ...
    model_name = (
        input("\nModel name (default 'game_phase_kmeans'): ").strip()
        or "game_phase_kmeans"
    )
    model_path = f"data/models/{model_name}.pkl"

    delay = float(
        input("Delay per bookmaker in seconds (default 0.5): ").strip() or "0.5"
    )
