...
class ColorCollector:
    ...
    def start_collection(self):
        ...
        for bookmaker, region in self.regions.items():
            t = threading.Thread(
                target=self._worker, args=(bookmaker, region, q), daemon=True
            )
            t.start()
            threads.append(t)
        ...
        try:
            while True:
                ...
                if time.time() - start >= 10:
                    ...
                    total += len(buffer)
                    self.logger.info(
                        f"Inserted {len(buffer)} records. Total: {total:,}"
                    )
...
def main():
    ...
    delay = float(
        input("Collection interval in seconds (default 0.01): ").strip() or "0.01"
    )
