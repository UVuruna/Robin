...
class PredictionAgent(BaseAviatorApp):
    ...
    def __init__(self):
        ...
        self.model_path = PATH.models_dir / "prediction_ensemble.pkl"
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for bookmaker in df["bookmaker"].unique():
            ...
            # Player behavior
            bm_df["players_mean_5"] = (
                bm_df["total_players"].rolling(5, min_periods=1).mean()
            )
            bm_df["players_std_5"] = (
                bm_df["total_players"].rolling(5, min_periods=1).std()
            )

            # Money patterns
            bm_df["money_mean_5"] = (
                bm_df["total_money"].rolling(5, min_periods=1).mean()
            )
    ...
    def prepare_training_data(
        self, df: pd.DataFrame, target_col: str = "target_high"
