...
class RGBTrainingDatabase(DatabaseModel):
    ...
    def update_labels(self, table: str, label_mapping: dict) -> int:
        ...
        cursor.executemany(f"UPDATE {table} SET label = ? WHERE id = ?", updates)
...
class BettingHistoryDatabase(DatabaseModel):
    ...
    def create_session(
        self, bookmaker: str, starting_balance: float, strategy: str
    ) -> int:
    ...
    def close_session(self, session_id: int, ending_balance: float) -> None:
