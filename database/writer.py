...
from config.config import AppConstants
...
class DatabaseWriter(Database):
        ...
        try:
            with self.lock:
                with self.transaction():
                    ...
                    self.logger.debug(
                        f"Inserted round {round_id} for {data['main']['bookmaker']}"
                    )
                    return round_id
    ...
    def _insert_snapshots(
        self, round_id: int, snapshots_data: List[Dict[str, Any]]
    ) -> None:
        ...
        try:
            with self.lock:
                with self.transaction():
                    for round_data in rounds_data:
                        try:
                            ...
                            self._insert_snapshots(
                                round_id, round_data.get("snapshots", [])
                            )
                    ...
                    self.logger.info(
                        f"Batch insert: {len([r for r in round_ids if r])}/{len(rounds_data)} successful"
                    )
