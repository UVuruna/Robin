    ...
    def benchmark_queries(self) -> Dict:
        ...
        try:
            ...
            queries = [
                ...
                (
                    "Recent rounds",
                    "SELECT * FROM rounds ORDER BY timestamp DESC LIMIT 100",
                ),
                (
                    "Bookmaker filter",
                    "SELECT * FROM rounds WHERE bookmaker = 'BalkanBet' LIMIT 100",
                ),
                (
                    "Join with earnings",
                    "SELECT r.*, e.* FROM rounds r JOIN earnings e ON r.round_ID = e.round_ID LIMIT 100",
                ),
            ]
...
def main():
    ...
    parser.add_argument(
        "--skip-vacuum", action="store_true", help="Skip VACUUM (faster)"
    )
