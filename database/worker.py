...
from config.config import app_config, performance_config  #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
from utils.logger import AviatorLogger, log_exception  #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
class DatabaseWorker(threading.Thread):
    ...
    def run(self) -> None:
        ...
        self.logger.info(
            f"Database worker started - Batch size: {self.batch_size}, "
    ...
    def _process_loop(self) -> None:
        ...
        while self.is_running or not self.db_queue.empty():
            try:
                ...
                if (
                    time.time() - last_stats_time
                    >= performance_config.stats_report_interval
                ):
    ...
    def _add_to_batch(self, item: Tuple[str, Dict]) -> None:
        ...
        try:
            ...
            # Add to batch
            self._pending_batch.append({"bookmaker": bookmaker_name, "data": data})
    ...
    def _process_batch(self) -> None:
        ...
        try:
            ...
            # Log success
            self.logger.debug(
                f"✅ Batch processed: {batch_size}/{self.batch_size} items "
        ...
        """,
            [
                (
                    s.get("round_ID"),
                    s.get("current_score"),
                    s.get("current_players"),
                    s.get("current_players_win"),
                )
                for s in snapshots
            ],
        )
    ...
    def _log_final_stats(self) -> None:
        ...
        if stats.total_batches > 0:
            self.logger.info(
                f"Avg batch size:      {stats.average_batch_size:.1f} items"
            )
            self.logger.info(
                f"Avg batch time:      {stats.average_batch_time * 1000:.1f} ms"
            )
            self.logger.info(
                f"Throughput:          {stats.items_per_second:.1f} items/sec"
            )

            # Calculate efficiency
            theoretical_max = stats.total_processing_time * (
                self.batch_size / self.batch_timeout
            )
            efficiency = (
                (stats.total_processed / theoretical_max * 100)
...
__all__ = ["DatabaseWorker", "WorkerStats", "get_worker", "stop_worker"]
