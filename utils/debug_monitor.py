...
class DebugMonitor:
    ...
    def _read_and_log(self):
        ...
        try:
            ...
            phase_name = (
                GamePhase(phase).name if phase != GamePhase.UNKNOWN else "UNKNOWN"
            )
            ...
            count_str = (
                f"{count_data.get('currently_playing', 0)}/{count_data.get('total_players', 0)}"
                if count_data
                else "N/A"
            )
            ...
            other_money_str = (
                f"{other_money_data.get('total_money', 0):.2f}"
                if other_money_data
                else "N/A"
            )
...
def main():
    ...
    try:
        ...
        interval = float(
            input("Read interval in seconds (default 0.5): ").strip() or "0.5"
        )
