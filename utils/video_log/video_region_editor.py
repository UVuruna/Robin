...
class VideoRegionEditor:
    ...
    def open_video(self) -> bool:
        ...
        print(
            f"   {int(self.cap.get(cv2.CAP_PROP_FRAME_WIDTH))}x{int(self.cap.get(cv2.CAP_PROP_FRAME_HEIGHT))} @ {self.fps:.1f}fps"
        )
        print(
            f"   {self.total_frames} frames ({self.total_frames / self.fps / 60:.1f} min)\n"
        )
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if self.drawing and self.start_point:
                ...
                if width > 10 and height > 10:
                    ...
                    print(
                        f"✅ Region {len(self.regions)}: {region_name} ({x_min},{y_min}) {width}x{height}"
                    )
    ...
    def draw_overlay(self) -> None:
        ...
        if self.rename_mode:
            info.append("")
            info.append(
                f"RENAME MODE: '{self.rename_buffer}_' (ENTER=save, ESC=cancel)"
            )
        ...
        for line in info:
            color = (
                (255, 255, 0)
                if "RENAME" in line or "UNSAVED" in line
                else (200, 200, 200)
            )
            cv2.putText(
                display, line, (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1
            )
            y_pos += 20
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Video region editor - ZERO terminal input!"
    )
    ...
    if output_path:
        ...
        print(
            f"Usage: python utils/video_screenshot_extractor.py <video> --regions-config {output_path}"
        )
    else:
