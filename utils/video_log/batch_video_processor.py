...
from utils.video_log.video_screenshot_extractor import VideoScreenshotExtractor
...
class BatchVideoProcessor:
    ...
    def get_video_files(
        ...
        if not all_videos and pattern == "*.mp4":
            print("⚠️  Nema .mp4 fajlova, tražim sve video formate...")
        ...
        if start < 0 or start >= len(all_videos):
            print(
                f"⚠️  Start index {start_index} van opsega! Ima {len(all_videos)} fajlova."
            )
            start = 0

        if end > len(all_videos):
            print(
                f"⚠️  End index {end_index} van opsega! Ima {len(all_videos)} fajlova."
            )
            end = len(all_videos)
    ...
    def process_with_progress(
        ...
        if not video_files:
            self._log("❌ Nema video fajlova u opsegu!")
        ...
        if start_index or end_index:
            print(
                f"📊 Opseg:         [{start_index or 1} - {end_index or '?'}] (po abecedi)"
            )

        print("⏱️  Intervali:     4, 16, 28, 40, 52 min (12 min razlika) @ 10 FPS")
        print(
            f"🎯 Završni frame: {'DA (SAMO za poslednji u batch-u)' if self.include_final_frame else 'NE'}"
        )
        print("📋 Break separatori: DA (iznad svakog screenshot-a)")
        ...
        print("📊 Output:        data/video_screenshots/")
        ...
        if start_index or end_index:
            self._log(
                f"📊 Opseg: [{start_index or 1} - {end_index or len(video_files)}]"
            )
        self._log("⏱️  Intervali: 4, 16, 28, 40, 52 min (HARDCODED @ 10 FPS)")
        self._log(f"{'=' * 70}")
        ...
        for i, video_path in enumerate(video_files, 1):
            ...
            print(f"\n{'─' * 70}")
            print(
                f"[{i}/{len(video_files)}] (#{global_idx}) Processing: {video_path.name}"
            )
            if include_final_for_this_video:
                print(
                    "🎯 POSLEDNJI U BATCH-U - dodaje se završni screenshot (30 frejmova pre kraja)!"
                )
            print(f"{'─' * 70}")
            ...
            try:
                ...
                collage_paths = self.extractor.process_single_video(
                    video_path, include_final_frame=include_final_for_this_video
                )
                ...
                self._log(
                    f"  ✅ Uspešno za {video_time:.1f}s - {len(collage_paths)} kolaža (OBRNUTO + Break separatori)"
                )
                ...
                if self.rename_suffix:
                    ...
                    if renamed_path:
                        ...
                        self.renamed_files.append(
                            {"original": video_path.name, "renamed": renamed_path.name}
                        )
        ...
        print(f"\n{'=' * 70}")
        print(
            f"🎨 Kreiranje MASTER kolaža za sve regione (max {self.max_collage_height}px)..."
        )
        print(f"{'=' * 70}")

        master_paths = self.extractor.create_master_collages(
            max_height=self.max_collage_height
        )
    ...
    def _generate_summary(self, master_collages_count: int = 0):
        ...
        total_collages = sum(
            r.get("collages", 0) for r in self.results if r["status"] == "success"
        )
        ...
        print(
            f"🎨 MASTER kolaža:  {master_collages_count} (svi screenshot-ovi + Break)"
        )
        print(f"⏱️  Ukupno vreme:    {total_time / 60:.1f} minuta")
        print(f"⚡ Avg po video:    {total_time / len(self.results):.1f} sekundi")
        print("📸 Intervali:      4, 16, 28, 40, 52 min (HARDCODED @ 10 FPS)")
        if self.rename_suffix:
            print(
                f"🏷️  Preimenovano:    {renamed_count}/{successful} fajlova (suffix: '-{self.rename_suffix}')"
            )
        print("=" * 70)
        ...
        self._log(f"\n{'=' * 70}")
        self._log(
            f"📊 SUMMARY: {successful} success, {failed} failed, "
            f"{total_time / 60:.1f}min, {master_collages_count} MASTER kolaža"
        )
        if self.rename_suffix:
            self._log(
                f"🏷️  Preimenovano: {renamed_count} fajlova sa '-{self.rename_suffix}'"
            )
        self._log(f"{'=' * 70}\n")
...
NAPOMENA:
  ...
  --interval opcija je uklonjena jer se više ne koristi!
    ...
    result = processor.process_with_progress(
        pattern=args.pattern, start_index=args.from_index, end_index=args.to_index
    )
