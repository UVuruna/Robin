...
class VideoScreenshotExtractor:
    ...
    def __init__(
        self, regions_config_path: str = "data/coordinates/video_regions.json"
    ):
    ...
    def extract_frames_at_intervals(
        self, video_path: Path, include_final_frame: bool = False
        ...
        print(
            f"⏱️  Trajanje: {duration_minutes:.2f} min ({total_frames} frejmova @ 10 fps)"
        )
    ...
    def create_vertical_collage_with_timestamps(
        ...
        # Kreiraj kolaž
        collage = Image.new(
            "RGB", (total_width, total_height), (bg_color[2], bg_color[1], bg_color[0])
        )
        ...
        draw.text(
            (20, 65),
            f"{len(pil_images)} screenshot-ova",
            fill=(180, 180, 180),
            font=font_small,
        )
        ...
        for i, (img, timestamp) in enumerate(zip(pil_images, timestamps)):
            ...
            draw.text(
                (x_offset + 5, break_y), break_text, fill=(255, 255, 0), font=font_break
            )
    ...
    def process_single_video(
        ...
        frames, time_points, fps = self.extract_frames_at_intervals(
            video_path, include_final_frame=include_final_frame
        )
        ...
        for region_name, images in regions_over_time.items():
            ...
            collage = self.create_vertical_collage_with_timestamps(
                pil_images, timestamps, region_name
            )
    ...
    def create_master_collages(self, output_dir: Path = None, max_height: int = 6000):
        ...
        for region_name, data_list in self.master_data.items():
            ...
            parts = self._split_into_parts(images, timestamps, max_height=max_height)
            ...
            if len(parts) == 1:
                ...
                print(
                    f"  ✅ {output_filename} ({len(images)} screenshot-ova, {parts[0]['final_height']}px)"
                )
                ...
                for part_idx, part in enumerate(parts, 1):
                    ...
                    print(
                        f"  ✅ {output_filename} ({len(part['images'])} screenshot-ova, {part['final_height']}px)"
                    )

                master_paths[region_name] = part_paths
                print(
                    f"  📊 {region_name}: {len(parts)} delova, {len(images)} total screenshot-ova"
                )
    ...
    def batch_process_videos(
        ...
        print("📸 Intervali: 4, 16, 28, 40, 52 min (12 min razlika)")
        print(
            f"🎯 Završni frame: {'DA (30 frejmova pre kraja)' if include_final_frame else 'NE'}"
        )
        print("✂️  Auto-split: Master kolaži > 6000px dele se na delove")
        print(f"{'=' * 70}")
        ...
        for i, video_path in enumerate(video_files, 1):
            ...
            if include_final:
                print(
                    "🎯 POSLEDNJI - dodaje završni screenshot (30 frejmova pre kraja)!"
                )

            try:
                collage_paths = self.process_single_video(
                    video_path, include_final_frame=include_final
                )
                ...
                print(f"  ❌ Greška: {e}")
                results.append(
                    {"video": video_path.name, "status": "failed", "error": str(e)}
                )
        ...
        print(
            f"🎨 Master kolaži: {len(master_paths)} regiona, {total_master_files} fajlova"
        )
...
if __name__ == "__main__":
    ...
    if args.single:
        ...
        extractor.process_single_video(video_path, include_final_frame=args.final_frame)
