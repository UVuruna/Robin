...
class PerformanceAnalyzer:
    ...
    def analyze_timeline(self, hours: int = 24) -> pd.DataFrame:
        ...
        """,
            (hours * 3600 * 10,),
        )  # Estimate 10 records per second max
    ...
    def calculate_throughput(self) -> Dict:
        ...
        throughput["efficiency"] = (
            throughput["records_per_second"] / expected_per_sec
        ) * 100
    ...
    def analyze_score_distribution(self) -> Dict:
        ...
        return {
            ...
            "high_percentage": (result[3] / (result[3] + result[4]) * 100)
            if result[3]
            else 0,
        }

    def compare_with_expectations(
        self, num_bookmakers: int, interval: float = 0.2
    ) -> Dict:
    ...
    def print_report(self, detailed: bool = False):
        ...
        if "first_record" in stats:
            print(
                f"   First record:         {stats['first_record'].strftime('%Y-%m-%d %H:%M:%S')}"
            )
            print(
                f"   Last record:          {stats['last_record'].strftime('%Y-%m-%d %H:%M:%S')}"
            )
        ...
        if "error" not in throughput:
            ...
            print(
                f"   Records/day:          {throughput['records_per_hour'] * 24:,.0f}"
            )
        ...
        if scores["avg_score"]:
            ...
            print(
                f"   High scores (≥2.0x):  {scores['high_scores']:,} ({scores['high_percentage']:.1f}%)"
            )
            print(
                f"   Low scores (<2.0x):   {scores['low_scores']:,} ({100 - scores['high_percentage']:.1f}%)"
            )
        ...
        if detailed and stats["total_records"] > 0:
            ...
            if not timeline.empty:
                pivot = timeline.pivot_table(
                    index="hour", columns="bookmaker", values="count", fill_value=0
                )
        ...
        if stats["unique_bookmakers"] > 0:
            ...
            if "error" not in comparison:
                ...
                print(
                    f"   Expected throughput:  {comparison['expected']['per_hour']:,.0f} records/hour"
                )
                print(
                    f"   Actual throughput:    {comparison['actual']['per_hour']:,.0f} records/hour"
                )
                print(
                    f"   Efficiency:           {comparison['efficiency']['percentage']:.1f}%"
                )

                if comparison["efficiency"]["lost_records_per_hour"] > 0:
                    print(
                        f"   ⚠️  Lost data:         {comparison['efficiency']['lost_records_per_hour']:,.0f} records/hour"
                    )
                    print(
                        f"   ⚠️  Lost data:         {comparison['efficiency']['lost_records_per_day']:,.0f} records/day"
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze Aviator data collection performance"
    )
    parser.add_argument("--db", default="aviator.db", help="Database path")
    parser.add_argument(
        "--detailed", action="store_true", help="Show detailed timeline"
    )
