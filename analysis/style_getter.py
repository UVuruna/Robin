...
class BettingStyleCalculator:
    ...
    def generate_betting_order(
        self, auto_cashout: float, win_per_round_order: List[int]
    ) -> List[int]:
        """
        ...
        if DEBUG:
            print(
                f"Auto cashout: {auto_cashout}, win per round order: {win_per_round_order}"
            )
        ...
        while True:
            ...
            bet = self.calculate_bet_amount(
                auto_cashout, win_per_round, total_invested, sequence
            )
...
def test_betting_styles(
    ...
    for auto_int in range(start_int, end_int + 1, step_int):
        ...
        betting_order = calculator.generate_betting_order(
            auto_cashout, win_per_round_order
        )
        ...
        detail = input("Do you want detailed output: ")
