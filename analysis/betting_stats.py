...
class BettingStatsAnalyzer:
    ...
    def analyze_bookmaker(self, bookmaker_name: str) -> BookmakerStats:
        ...
        for i, bet in enumerate(working_table):
            ...
            if score >= self.config.auto_cashout:
                ...
                # Brojač dobitaka
                counter_key = str(
                    min(current_loss_counter + 1, self.config.max_loss_streak)
                )
    ...
    def print_bookmaker_summary(self, stats: BookmakerStats):
        ...
        trim_info = (
            f" [trimmed: {stats.trimmed_rounds}]" if stats.trimmed_rounds > 0 else ""
        )
        final = (
    ...
    def print_final_summary(self, bookmaker_summaries: List[str]):
        ...
        trim_info = (
            f" | Trimmed: {self.total_trimmed}" if self.total_trimmed > 0 else ""
        )
...
if __name__ == "__main__":
    ...
    config = BettingConfig(
        ...
        if MAX_LOSS <= len(BETTING_ORDER)
        else len(BETTING_ORDER),
        full_output=True,
    )
