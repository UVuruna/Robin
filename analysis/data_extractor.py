...
class OverlapDetector:
    ...
    def remove_overlaps(chunks: List[ChunkData]) -> List[ChunkData]:
        ...
        for i in range(len(chunks) - 1):
            ...
            overlap_len = OverlapDetector.find_overlap_length(
                upper.scores, lower.scores
            )

            chunk_time = (
                upper.timestamp.strftime("%Y-%m-%d %H:%M:%S")
                if upper.timestamp
                else f"#{i}"
            )
            ...
            if overlap_len > 0:
                ...
                print(
                    f"  Chunk {chunk_time}: Uklonjeno {overlap_len} preklapajucih skorova"
                )
            else:
...
class TimeInterpolator:
    ...
    def calculate_times(chunks: List[ChunkData]) -> List[Dict]:
        ...
        for i in range(len(chunks) - 1):
            ...
            print(
                f"  Chunk {chunk_time}: {num_scores} skorova, {time_diff:.0f}s razlika, avg={avg_sec:.2f}s"
            )

            for j, score in enumerate(current_chunk.scores):
                current_time = current_chunk.timestamp - timedelta(seconds=j * avg_sec)

                results.append({"score": score, "time": current_time, "sec": None})
        ...
        print(
            f"  Poslednji chunk {last_chunk_time}: {len(last_chunk.scores)} skorova, avg={global_avg_sec:.2f}s"
        )
...
class DataExtractorV2:
    ...
    def __init__(
        self, logs_dir: str = "analysis/txt", output_dir: str = "analysis/csv"
    ):
